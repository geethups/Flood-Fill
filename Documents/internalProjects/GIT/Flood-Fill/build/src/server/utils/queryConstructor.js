...
var dbSchema = cfgManager.getConfig('dbSchema');
...
function getValues(obj) {
    try {
        var returnValue = function returnValue(k) {
            return '\'' + mysqlRealEscapeString(obj[k]) + '\'';
        };

        var valueList = Object.keys(obj).map(returnValue);
...
function getQueryString(table, data, DBoperation) {
    ...
    try {
        if (DBoperation === 'INSERT') {
            var column = getColumns(getColumnsForTable(table));
...
function getSelectQuesQueryString(table, quesDesc) {
    ...
    if (quesDesc === 'all') {
        ...
        var value = '\'' + mysqlRealEscapeString(quesDesc) + '\'';
...
function getSelectLoginQueryString(table, loginData) {
    var query;
    query = 'SELECT * FROM ' + table + ' where userName = ' + '\'' + mysqlRealEscapeString(loginData.username) + '\'' + ' and password=' + '\'' + mysqlRealEscapeString(loginData.password) + '\'' + ';';
    return query;
}
...
//# sourceMappingURL=queryConstructor.js.map
